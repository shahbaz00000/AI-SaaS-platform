...
const WriteArticle = () => {
  ...
  const {generateArticle,isLoading,isError,articleContent} = useArticle();
  console.log(isLoading,articleContent)

  const handleArticleForm = (e)=>{
    e.preventDefault();

    const articlePrompt = articleRef.current.value;

    generateArticle(articlePrompt);
  }
    ...
    <div className='min-h-[calc(100vh-64px)] bg-gray-50 p-6'>
      <div className='max-w-7xl mx-auto space-y-6'>
        ...
        <div className='grid grid-cols-1 xl:grid-cols-[420px_1fr] gap-6'>
          <form onSubmit={handleArticleForm} className='bg-white rounded-3xl border border-gray-200 p-6 shadow-sm'>
