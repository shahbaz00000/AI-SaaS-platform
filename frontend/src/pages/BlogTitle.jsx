...
import useBlogTitle from '../hooks/blogTitleHook'

const BlogTitle = () => {
  const blogTitleRef = useRef()
  const { generateBlogTitle } = useBlogTitle()

  const handleBlogTitleForm = (e) => {
    e.preventDefault()

    const blogTitle = blogTitleRef.current.value
    generateBlogTitle(blogTitle)
  }
    ...
    <div className='min-h-[calc(100vh-64px)] bg-gray-50 p-6'>
      <div className='max-w-7xl mx-auto space-y-6'>
        ...
        <div className='grid grid-cols-1 xl:grid-cols-[420px_1fr] gap-6'>
          <form onSubmit={handleBlogTitleForm} className='bg-white rounded-3xl border border-gray-200 p-6 shadow-sm'>
            <h2 className='text-xl font-semibold text-gray-900 mb-4'>Enter prompt</h2>
            <p className='text-sm text-gray-500 mb-6'>Type your blog prompt below to generate title suggestions.</p>
          ...
          <section className='bg-white rounded-3xl border border-gray-200 p-6 shadow-sm min-h-320px'>
            ...
            <div className='rounded-3xl border border-dashed border-gray-300 bg-gray-50 p-5 text-gray-600 min-h-220px'>
              <p className='text-gray-500'>Your generated output will show here.</p>
            </div>
          </section>
        </div>
      </div>
